...
do
    ...
    then 
        ...
        echo $info_color"INFO"$no_color": Initializing the cluster with node#"$i
        winpty docker exec -d --privileged rp$i "/opt/redislabs/bin/rladmin" cluster create name $rp_fqdn username $rp_admin_account_name password $rp_admin_account_password flash_enabled
    else
        #added nodes
        ...
        winpty docker exec -d --privileged rp$i "/opt/redislabs/bin/rladmin" cluster join username $rp_admin_account_name password $rp_admin_account_password nodes 10.0.0.2 flash_enabled
    fi
 done
